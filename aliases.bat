...
doskey udahk=powershell -Command "Start-Process cmd -Verb RunAs -ArgumentList '/k cd /d %CD% && taskkill /im "autohotkey.exe" && taskkill /im "autohotkey64.exe" && winget upgrade autohotkey.autohotkey && del "%userprofile%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\script.ahk" && curl https://raw.githubusercontent.com/oscarmacas/scripts/main/script.ahk$G"%userprofile%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\script.ahk" && "%userprofile%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\script.ahk"
